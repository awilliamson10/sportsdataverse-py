...
def espn_mbb_pbp(game_id: int, raw=False) -> Dict:
    ...
    summary_url = "http://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/summary?event={}".format(
        game_id
    )
    ...
    pbp_json = helper_mbb_pbp(game_id, pbp_txt)
    return pbp_json
...
def helper_mbb_pbp_features(
    ...
    pbp_txt["plays"]["start.quarter_seconds_remaining"] = 60 * pbp_txt["plays"]["clock.minutes"].astype(int) + pbp_txt[
        "plays"
    ]["clock.seconds"].astype(int)
