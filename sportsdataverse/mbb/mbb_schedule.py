...
def espn_mbb_schedule(dates=None, groups=50, season_type=None, limit=500) -> pd.DataFrame:
    ...
    url = "http://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/scoreboard?limit={}{}{}{}".format(
        limit, dates, groups, season_type
    )
    ...
    if resp is not None:
        ...
        for event in events:
            ...
            del_keys = ["geoBroadcasts", "headlines", "series", "situation", "tickets"]
            for k in del_keys:
                event.get("competitions")[0].pop(k, None)
            # check if odds are available
            if event.get("competitions")[0].get("odds") is None:
                event.get("competitions")[0]["odds"] = False
...
def espn_mbb_calendar(season=None, ondays=None) -> pd.DataFrame:
    ...
    if ondays is not None:
        url = "https://sports.core.api.espn.com/v2/sports/basketball/leagues/mens-college-basketball/seasons/{}/types/2/calendar/ondays".format(
            season
        )
        resp = download(url=url)
        ...
        url = "http://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/scoreboard?dates={}".format(
            season
        )
        resp = download(url=url)
        ...
        data = {"season": season, "datetime": txt, "date": date, "year": year, "month": month, "day": day, "dateURL": datenum}
